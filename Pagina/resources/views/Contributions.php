...
$results=$con->runQuery('SELECT contributions from user_information WHERE email=$1',[$_SESSION['user_email']]);
$contador=1;
$numero_empresa=1;
$array=explode(",",$results[0]['contributions']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if($Name!=''||$Description!=''||$Url!=''){
		$texto.='Name='.$Name.',Description='.$Description.',Url='.$Url;
	}else{
		 $texto = trim($texto, ',=,');
	}
	$con->runStatement('UPDATE user_information set contributions=$1 where email=$2',[$texto,$_SESSION['user_email']]);	
...
?>
<h2>Contributions</h2>
<form method="POST" action="/Contributions.php">
<?php
if($results&&$results[0]['contributions']!=""){
	foreach ($array as $key) { 
	$var=explode("=",$key);
	if ($contador==1) {
...
<div class="field is-grouped">
    ...
    <div class="control">
        <button type="button" name="Cancelar" class="button is-link " onClick="location.href='/Contact.php'">Siguiente</button>
    </div>
</div>
</form>
